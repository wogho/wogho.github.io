...
GitHub에는 포트폴리오 템플릿이 수백 개 있습니다. 그런데 **Next.js + i18n + 다크 모드 + 경력 상세 페이지**를 모두 갖춘 템플릿은 거의 없습니다.

이 템플릿이 다른 이유:

- **경력 상세 페이지가 있습니다.** 단순한 타임라인이 아닌, 프로젝트별 배경/역할/성과/교훈을 서술하는 전용 `/career` 페이지. 시니어 엔지니어에게 꼭 필요한 기능인데 대부분의 템플릿에는 없습니다.
- **다국어가 처음부터 설계되어 있습니다.** 나중에 끼워 넣은 게 아닙니다. `next-intl` 기반으로 한국어/영어가 URL 라우팅까지 완전히 분리되어 있습니다.
- **설정 파일 하나로 완성됩니다.** `src/config/site.ts` 하나와 데이터 파일 7개만 수정하면 됩니다. 컴포넌트 코드를 건드릴 필요가 없습니다.
- **디자인 품질이 다릅니다.** Framer Motion 애니메이션, 그라데이션 오브, 부드러운 스크롤 인디케이터. 직접 만들면 몇 주 걸릴 수준입니다.

---

## 주요 기능

- **다국어 지원 (i18n)** — `next-intl` 기반 한국어/영어 기본 제공. URL 라우팅(`/ko`, `/en`)까지 완전히 분리되어 있어 다른 언어도 쉽게 추가할 수 있습니다.
- **다크 / 라이트 모드** — `next-themes`로 시스템 설정을 자동 감지하고, 수동 토글도 지원합니다. 전환 시 깜빡임이 없습니다.
- **Framer Motion 애니메이션** — 카드 진입 효과, 플로팅 오브, 부드러운 스크롤 인디케이터. `LazyMotion`으로 번들 크기를 최소화했습니다.
- **경력 상세 페이지** — 단순한 목록이 아닌, `/career` 페이지에서 프로젝트 배경/역할/성과/교훈을 상세히 서술할 수 있습니다. 시니어 포지션 지원에 특히 유용합니다.
- **SEO 최적화** — JSON-LD 구조화 데이터, Open Graph 이미지, sitemap, robots.txt가 모두 포함되어 있습니다.
- **완전 반응형** — 모바일 우선 설계. 햄버거 메뉴, 터치 제스처, 모든 화면 크기에서 최적화됩니다.
- **설정 파일 하나** — `src/config/site.ts` + 데이터 파일만 수정하면 됩니다. 컴포넌트 코드를 건드릴 필요가 없습니다.
- **접근성** — ARIA 라벨, 키보드 내비게이션, 포커스 관리를 갖추고 있습니다.
- **성능** — LazyMotion, CSS 키프레임 애니메이션, `next/font` 최적화로 빠른 로딩을 보장합니다.
